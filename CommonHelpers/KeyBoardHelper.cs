...
namespace TMHelper.Common
{
    ...
    {
        public static uint KeyBoardKeyPress(KeyCode[] keys)
        {
            uint iResult = 0;
            ...
            try
            {
                ...
                iResult = SendInput(Convert.ToUInt32(inputs.Count()), inputs, Marshal.SizeOf(typeof(INPUT)));
            }
            catch (Exception ex)
            {
                iResult = 99;
        ...
        public static uint KeyBoardKeyPress(string strInput)
        {
            uint iResult = 0;
